...
```bash
theHarvester -d <tên_miền_web> -l 500 -b google,linkedin,bing
```
...
```bash
metagoofil -d <tên_miền_web> -t pdf -l 100 -n 25 -o pdfs -f pdf.html
```
...
```bash
site:<tên_miền_web> intitle:login
```
```bash
site:<tên_miền_web> filetype:php | filetype:asp
```
